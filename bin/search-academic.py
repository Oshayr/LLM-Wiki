#!/usr/bin/env python3
"""search-academic.py — Academic paper search across multiple APIs.

Usage:
    python3 wiki-academic-search.py search <query> [--top N] [--year-min YYYY] [--year-max YYYY]
    python3 wiki-academic-search.py cite <doi_or_title>
    python3 wiki-academic-search.py download <doi_or_url> --output <path>

APIs (tried in order, free tier / no key required):
    1. Semantic Scholar API (api.semanticscholar.org)
    2. OpenAlex API (api.openalex.org)
    3. CrossRef API (api.crossref.org)
    4. arXiv API (export.arxiv.org)
"""
